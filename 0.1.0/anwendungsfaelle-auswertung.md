...
1. BET-Rate (brusterhaltend vs. Mastektomie)
1. R0-Rate aus OP-Outcomes
1. Altersverteilung der Patientinnen (Boxplot)
1. Therapie-Mix pro Subtyp (Chemo / endokrin / zielgerichtet / Immun)
1. Time-to-Treatment (Diagnose → OP) Verteilung
...
1. **`"python"` — Pathling Python-Bibliothek**: lokale Ausführung über`PathlingContext`mit eingebettetem Spark; es ist kein Docker nötig (`pip install pathling`).
1. **`"custom"` — eingebauter FHIRPath-Mini-Runner**: Fallback ohne Pathling/Spark, nutzt den HAPI-Testserver oder direkt die JSON-Ressourcen aus`fsh-generated/resources/`.
